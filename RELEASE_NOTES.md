...
- **`active` is no longer overridden by the router on a `routerLink` item.** The item's link carried both `[class.active]="active()"` and `routerLinkActive="active"`, and the directive removes the class as well as it adds it: whichever wrote last won. `[active]="true"` did not light an item whose link differs from the URL (a docs entry on its deeper pages), and `[active]="false"` did not keep off one whose link is a prefix of the URL (a link to the root lit on every page, next to the right item). Left unset, `active` still lets the router decide; set, it now decides alone.
